...
そのため、学生側では全7回または8回の授業であるが、
教員側では合計14回または16回の授業実施となる。
...
def inspect_work():
    ...
    if object_id in (1, 2):
        ...
        # TODO: RETRY を表示 / 安全側として Relay ON
        pass
...
とする。
授業上は「AIが分からない」と「NG」を区別して表示してもよいが、Relay出力は安全側へ倒す。
...
- `050_huskylens_m5_inspection.md`
- `050_huskylens_m5_inspection.pdf`
- `051_ai_learning_m5_connection.md`
- `051_ai_learning_m5_connection.pdf`
- `052_...` main.py テンプレート

を GitHub へ commit 済み。

第5回は、

> HUSKYLENS 4クラス学習  
> → M5StickC PLUS2 で ID取得  
> → Button A を検査開始入力として使用  
> → OK / NG / RETRY 判定  
> → LCD表示  
> → Relay Unit 出力
...
その後、Button A を実際のワーク検出センサへ置き換える方向で発展させる。
