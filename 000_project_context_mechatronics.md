...
# 47. 第7回1st turn 振り返り（2026-09-14）

## 47.1 実施できたこと

- A・Bグループで第7回1st turnを実施した。
- 森島は9/14に1H（50分）遅刻し、その1Hを欠課として扱う。
- 9:20頃にFA実習室へ移動し、学生ノートPCへThonnyを導入した。
- 今回のThonnyインストールは約5分で終了し、待ち時間を休憩として利用して9:35頃に再開した。
- 第6回で使用したIRセンサ・HUSKYLENS等の物理配置を再現した。
- 認識改善よりもPLC連携を優先する方針へ授業中に切り替えた。
- 既存ケーブルではPLC接続位置まで届かないことが分かり、延長ケーブル作製案を検討したが、PLC側の接続形態が判然としなかったため、その場では作製しなかった。
- 10:40頃からPLC接続へ向けた確認・配線調査へ移り、11:10～11:20頃に接続準備段階まで進んだ。
- PLCの実配線・端子台・I/O追跡に関する学生の経験が十分ではないことが分かり、安全と残り時間を考慮して、その日のPLC実接続は中止した。

## 47.2 想定と異なったこと

- 第6回2nd turnではThonny初回導入等に約30分を要したが、第7回1st turnでは約5分で終了した。PCの起動状態・個体差によって所要時間が大きく変動するため、固定時間として見積もらず余裕を持たせる。
- 学生はGX WorksによるPLCプログラム経験はあるが、FA設備の端子台・実配線・I/O配線追跡を実務的に行う経験は十分ではなかった。
- このFAラインは既存配線を固定的に使用する前提が強く、「学生が空きI/Oを探して自由に追加配線する」教材としては扱いにくい。

## 47.3 学生が詰まったところ

- 主なボトルネックはPythonやGX Worksの操作ではなく、PLCハードウェア側だった。
- 特に、端子台から実配線を追って使用可能なI/Oを判断し、安全な接続方法を決める作業は、第7回の200分実習内で学生に一から行わせるには過大だった。
- 今後のPLC連携は、教員側で安全な接続点・電圧・論理・端子位置を事前確認し、学生には指定された接続と動作確認を行わせる方式を基本とする。

## 47.4 第7回1st turnからの授業設計上の確定事項

- PLC接続点を学生自身に探索させる方式は採用しない。
- 学生には、確認済みの接続点を使って「M5判定 → Relay → PLC入力」というシステム統合を体験させる。
- PLC入力内部の詳細回路やNPNの理論説明は主課題とせず、必要な操作・観察に絞る。

# 48. 制御ボックス・PLC入力X00に関する現物確認（2026-09-14）

## 48.1 第5ステーションのX00センサ系

第5ステーションのX00に接続されている光電センサ系について、現物表示から次を確認した。

- ファイバユニット：OMRON `E32-T12R`
- ファイバアンプ：OMRON `E3X-DA11-S`

第5ステーションで、X00端子をN（0V）基準で実測した結果は次のとおり。

- ワークなし・受光状態：X00 約23 V
- ワークあり・遮光状態：X00 約52 mV（ほぼ0 V）

この実測挙動は、検出時にX00を0V側へ引き込むNPNシンク型の動作と整合する。

## 48.2 制御ボックスのX00手動入力

各ステーションにはPLC入力を模擬できる制御ボックスがあり、各制御ボックスの `X00` は対応するPLCの `X00` へ接続されている。水色のBit 0スイッチでPLC入力X00を手動ONできる。

一方、外部センサがX00系へ接続されているかどうか、またその接続形態はステーションごとに異なる。

制御ボックスから外部の `X00` / `N` プラグを抜き、制御ボックス側メス端子の `X00-N` 間を測定した結果、

- 水色SW OFF：X00-N 約24 V
- 水色SW ON：X00-N 約0 V

となった。

制御ボックス表面の印刷回路図とこの実測から、水色スイッチと外部入力（接点・半導体）は `X00-N` 間に並列接続されていると判断する。

学生向けには、次の理解までを標準とする。

> この設備では、X00とNをつなぐとPLC入力X00がONになる。
> 水色スイッチも、センサーも、Relay接点も、PLCから見れば同じ役割をする。
> 制御ボックスではそれらが並列につながっている。

PLC内部入力回路、フォトカプラ、NPNシンクの詳細は、授業の本線では扱わない。

## 48.3 RelayによるX00入力

M5 Relay Unitの無電圧接点を、制御ボックスの `X00-N` 間へ接続すれば、水色スイッチと同様の入力を作れる。
...
Relay接点自体に極性はないためCOM/NOの向きを逆にしても接点機能は同じだが、授業では配線を統一するため、白=X00→NO、黒=N→COMを標準とする。

期待する動作は次のとおり。

- Relay OFF：X00-N 開放、X00 約24 V、PLC X00 OFF
- Relay ON：X00-N 導通、X00 約0 V、PLC X00 ON

授業前に教員側で、Relay OFF/ONとGX Works上のX00 OFF/ONが一致することを確認する。

# 49. 第7回2nd turn（2026-09-17）に向けた確定準備・方針

## 49.1 授業ゴール

第7回2nd turnの最終ゴールは1st turnと同じく、

> **HUSKYLENS → M5StickC PLUS2 → Relay Unit → PLC入力 X00**

を1つのシステムとして動作させることとする。

C・Dグループは第6回で10回試験を完了できていないため、まず10回試験を完了し、その後PLC統合へ進む。

認識改善はPLC統合時間を圧迫しないよう、標準課題から外し、時間に余裕がある場合のみ1項目実施する。
...
- 「Relayはスイッチといっしょ」
- 「Relayはセンサーといっしょ」
- 「この制御ボックスでは、それらが並列につながる」
- 「X00とNをつなぐとPLCのX00がONになる」

マイコンのプルアップとの比較、PLC内部のフォトカプラや入力回路の詳細説明は行わない。

## 49.3 PLC接続用仮設ケーブル

制御ボックスの差し込みプラグについて、メカトロ担当教員へ確認したところ、予備プラグ部品はない。購入も第7回2nd turnに間に合わない可能性があるため、仮設ケーブルを教員側で準備した。
...
- N（GND）側：黒ケーブル × 4本
- 合計：8本
- 4グループ分（1st turn側の追実験・第8回利用も見込む）
...
- 一回折り返して差し込み端子へ挿入する
- 現物で保持できることを確認済み
- 接触部にはんだを含ませると太く・硬くなり端子へ入らなくなるため、はんだ付けしない
- 根元のみ収縮チューブで補強する

学生にケーブル工作はさせず、教員準備済みケーブルを使用させる。

## 49.4 第7回2nd turn配布資料の方針

配布資料 `070_inspection_improvement_plc_2nd.md` は、次の流れを本線とする。

1. FA実習室の環境復元・正本 `main.py` 確認
2. IR・HUSKYLENS・Relay・物理配置確認
3. 第6回で未完だった10回試験を完了
4. Class ID正答率・設備判断正答率・誤OKを整理
5. 水色スイッチでPLC X00のON/OFFを確認
6. 教員準備ケーブルでRelayをX00-Nへ接続
7. GX WorksでRelay OFF/ONとX00 OFF/ONを確認
8. 各IDを最低1回ずつ流し、M5判定→Relay→PLC X00の統合動作を記録
9. 時間が余れば認識改善を1項目のみ実施
10. 180分で結果整理・復旧・片付けへ移行

BBB / YYY / BBR / YYR の表記は、学生向け資料では次の注釈を付ける。

- B = Blue（青）
- Y = Yellow（黄）
- R = Red（赤）

STEP 6には、各IDを最低1回ずつ確認できる統合確認用記録フォームを設ける。
